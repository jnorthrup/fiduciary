...
export const CollateralManagementWidget: React.FC<Props> = ({ entity, onClose }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row h-full w-full overflow-hidden font-sans relative">
      ...
      <div className="flex-1 flex flex-col bg-white relative overflow-hidden">
          ...
          {isCreatingPool ? (
              <div className="flex-1 p-12 flex flex-col justify-center max-w-2xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 overflow-y-auto">
          ...
          {isPledging && (
              <div className="absolute inset-0 bg-white/90 backdrop-blur z-20 flex flex-col items-center justify-center p-8 animate-in fade-in">
                  <div className="bg-white border border-slate-200 shadow-2xl rounded-2xl p-8 max-w-lg w-full relative">
                      {/* Nested Close for Pledge Sub-modal */}
                      <button onClick={() => setIsPledging(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600">
                          <X size={20} />
                      </button>
