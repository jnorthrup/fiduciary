        ...
        <div className="fixed inset-0 z-[200] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="p-6 space-y-6">
                    ...
                    {/* Interface Settings */}
                    <div className="pt-4 border-t border-slate-100">
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Application Interface</label>
                        <button
                            onClick={() => {
                                const current = localStorage.getItem('fiduciary_skin_v2');
                                const next = current === 'lastrust' ? 'jnorthrup' : 'lastrust';
                                localStorage.setItem('fiduciary_skin_v2', next);
                                window.location.reload();
                            }}
                            className="w-full flex items-center justify-between p-4 rounded-xl border-2 border-slate-200 hover:border-indigo-500 hover:bg-indigo-50 group transition-all"
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div className="text-left">
                                    <h4 className="text-sm font-bold text-slate-800">Switch Application Skin</h4>
                                    <p className="text-xs text-slate-500">Toggle between Trust Ledger and Clear.Flow views.</p>
                                </div>
                            </div>
                            <div className="text-xs font-bold bg-white px-3 py-1 rounded border border-slate-200 group-hover:border-indigo-200 text-slate-600">
                                Switch
                            </div>
                        </button>
                    </div>
