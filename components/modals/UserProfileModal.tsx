...
import { User as UserIcon, X, Save, Lock, Unlock, Phone, Briefcase, Building, Mail, Trash2, Shield, EyeOff, RefreshCw, Cloud, Camera } from 'lucide-react';
            ...
            <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        <div className="h-12 w-12 rounded-full bg-slate-800 text-white flex items-center justify-center font-bold text-lg border-2 border-white shadow-sm overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                    ...
                    {formData.profileImage && (
                        <div className="flex items-center gap-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-100">
                            <div className="h-16 w-16 rounded-full overflow-hidden border-2 border-white shadow-lg ring-2 ring-blue-200">
                                <img src={formData.profileImage} alt={formData.name} className="w-full h-full object-cover" />
                            </div>
                            <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                    <Cloud size={14} className="text-blue-500" />
                                    <span className="text-xs font-bold text-blue-600 uppercase tracking-wide">Synced from Google</span>
                                </div>
                                <p className="text-xs text-slate-500">Profile picture automatically updated from your Google account</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="space-y-4">
                        <div>
                            <div className="flex justify-between items-center mb-1">
                                ...
                                {formData.profileImage && <span className="text-[10px] text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full font-bold">Synced</span>}
                            </div>
                            <input
                                type="text"
                                value={formData.name}
                                ...
                                className={`w-full border border-slate-300 rounded-lg p-2.5 text-sm outline-none ${!!formData.profileImage ? 'bg-slate-50 text-slate-500 cursor-not-allowed' : 'focus:ring-2 focus:ring-indigo-500'}`}
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <div className="flex justify-between items-center mb-1">
                                    ...
                                    {formData.profileImage && <Lock size={10} className="text-slate-400" />}
                                </div>
                                <input
                                    type="email"
