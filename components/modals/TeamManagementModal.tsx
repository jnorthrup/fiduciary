...
export const TeamManagementModal: React.FC<Props> = ({ users, currentUser, onAddUser, onUpdateUser, onDeleteUser, onEditUser, onClose }) => {
  ...
  const isAdmin = currentUser.role === 'Owner' || currentUser.role === 'Admin' || currentUser.role === 'Beneficial Owner';
  ...
  const getRoleBadge = (role: UserRole) => {
      switch(role) {
          case 'Owner': return <span className="bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full text-[10px] font-bold border border-purple-200 uppercase">OWNER</span>;
          case 'Beneficial Owner': return <span className="bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full text-[10px] font-bold border border-amber-200 uppercase">Beneficial Owner</span>;
          case 'Admin': return <span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full text-[10px] font-bold border border-indigo-200 uppercase">ADMIN</span>;
          case 'Editor': return <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full text-[10px] font-bold border border-blue-200 uppercase">EDITOR</span>;
          case 'Viewer': return <span className="bg-slate-100 text-slate-700 px-2 py-0.5 rounded-full text-[10px] font-bold border border-slate-200 uppercase">VIEWER</span>;
      }
  };
    ...
    <div className="fixed inset-0 z-[200] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            {isInviteMode ? (
                <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 mb-6 animate-in slide-in-from-top-2 relative">
                    ...
                    <div className="flex flex-col gap-4">
                        <div>
                            <label className="block text-xs font-bold text-slate-500 mb-2">Select Role</label>
                            <div className="flex flex-wrap gap-2">
                                {['Beneficial Owner', 'Admin', 'Editor', 'Viewer'].map((role) => (
                                    <button
                                        key={role}
                                        onClick={() => setNewRole(role as UserRole)}
                                        className={`px-3 py-1 text-xs rounded-full border transition-colors ${newRole === role ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm' : 'bg-white text-slate-500 border-slate-200 hover:border-indigo-300'}`}
                                    >
                                        {role}
                                    </button>
                                ))}
                        ...
                        <div className="flex justify-end gap-2 pt-2 border-t border-slate-200/50">
                            ...
                            <button onClick={handleInvite} className="bg-indigo-600 text-white px-6 py-2 rounded-lg text-xs font-bold hover:bg-indigo-700 shadow-md">Send Invite</button>
                        </div>
                    </div>
                </div>
            ) : (
                        ...
                        >
                            <div className="flex items-center gap-3">
                                <div className="relative">
                                    ...
                                    {user.isPrivate && (
                                        <div className="absolute -bottom-1 -right-1 bg-white rounded-full p-0.5 shadow-sm border border-slate-100" title="Private Profile">
        ...
        <div className="bg-slate-50 p-4 border-t border-slate-200 text-xs text-center text-slate-400">
            Admins and Beneficial Owners can edit any profile. Users can only edit their own. Private profiles hide activity details.
        </div>
      </div>
    </div>
  );
