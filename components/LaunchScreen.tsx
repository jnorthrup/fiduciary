...
export const LaunchScreen: React.FC<Props> = ({
  onLaunch, onSyntheticFuzz, onResumePersistent, onCreditUnionLaunch, canResume, onGoogleLogin
}) => {
  const { wipeSession, importData } = useLedgerStore();
  ...
  useEffect(() => {
    if (bootProgress >= 100) {
      const timer = setTimeout(() => {
        const timer = setTimeout(() => {
          if (activeStrategy === 'Fuzz') onSyntheticFuzz();
  ...
  }, [bootProgress, activeStrategy, onSyntheticFuzz, onResumePersistent, onCreditUnionLaunch, onLaunch, name, email]);

  const selectStrategy = (strategy: 'Manual' | 'Fuzz' | 'Resume' | 'CreditUnion') => {
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-8 duration-700">
              <StrategyCard
                ...
                desc="Securely access your ledger via Google OAuth."
              ...
              <StrategyCard
                icon={Sparkles}
                title="Try Demo Mode"
                desc="Explore the system with synthetic data."
            ...
            {canResume && (
              <div className="mt-8 pt-8 border-t border-slate-800 flex justify-center animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-200">
                <button
                  onClick={() => selectStrategy('Resume')}
                  className="group relative flex items-center gap-4 bg-slate-900 border border-slate-700 px-8 py-4 rounded-xl hover:border-slate-500 transition-all"
                >
                  <History className="text-slate-400 group-hover:text-white transition-colors" />
                  <span className="text-slate-400 font-bold group-hover:text-white transition-colors">Resume Previous Session</span>
                </button>
              </div>
            )}
          </>
