...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const cashData = await apiGet<any>('/charts/cash-balance');
        const transitData = await apiGet<any>('/charts/transit-balance');
        const apData = await apiGet<any>('/charts/ap-balance');
        setCash(cashData?.latest?.y ?? cashData?.latest ?? cashData?.value ?? 0);
        setTransit(transitData?.latest?.y ?? transitData?.latest ?? transitData?.value ?? 0);
            ...
            {navItems.map(item => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-semibold transition-colors ${
                  route.startsWith(item.route.split('/')[1] ? `/${item.route.split('/')[1]}` : item.route)
