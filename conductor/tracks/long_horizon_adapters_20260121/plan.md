...
**Status:** PLANNING
**Created:** 2026-01-21
**Scope:** Future-proof adapter architecture for storage, auth, crypto, and cloud-native patterns

---

## Overview

Long-horizon architectural patterns for pluggable adapters. These are **not current goals** but documented for future iteration when requirements emerge (USB crypto, multi-cloud, regulatory compliance, scalability).
...
## Phase 4: Cloud Native GCS Patterns (Serverless Maximization)

### Throughput & Cost Optimization
- [ ] **Signed URLs**: Bypass Cloud Run memory limits for large uploads (NACHA/W-2) by uploading directly to GCS
- [ ] **Composite Objects**: Use `bucket.combine()` for server-side WAL compaction (zero-copy concat)
- [ ] **Lifecycle Policies**: Auto-transition old WAL/snapshots to Nearline/Coldline (compliance vs cost)

### Event-Driven Architecture
- [ ] **GCS Notifications**: Trigger Pub/Sub events on file upload (decoupling ingestion from processing)
- [ ] **Object Versioning**: Enable bucket versioning for immediate "undo" capability on state files
...
2. **Stick to Google** - No multi-cloud for now, maximize Google ecosystem (GCS, Pub/Sub)
...
- [gcs-persistence.js](file:///Users/jim/work/fiduciary/server/lib/gcs-persistence.js) - Current GCS implementation
- [architecture-serverless.md](file:///Users/jim/work/fiduciary/docs/architecture-serverless.md) - OAuth audit, billing model, storage patterns
