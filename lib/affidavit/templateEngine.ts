...
function substituteVariables(
  ...
  htmlResult = htmlResult.replace(/\{#if\s+(\w+)\}([\s\S]*?)\{\/if\}/g, (_, varName, conditionalContent) => {
    const value = varMap[varName];
    return value ? conditionalContent : '';
  });
  result = htmlResult; // Apply same conditional logic to plain text

  // Handle loops: {#items} {{item.prop}} {/items}
  htmlResult = htmlResult.replace(/\{#(\w+)\}([\s\S]*?)\{\/\1\}/g, (_, arrayName, loopContent) => {
