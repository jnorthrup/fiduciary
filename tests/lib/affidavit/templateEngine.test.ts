...
describe('TemplateEngine - renderTemplate', () => {
  ...
  it('should escape HTML in variable values', async () => {
    ...
    // claimant_name is used in the second section (Capacity Statement)
    const capacitySection = rendered.sections.find(s => s.id === 'capacity');
    expect(capacitySection).toBeDefined();

    if (capacitySection) {
      expect(capacitySection.htmlContent).not.toContain('<script>');
      expect(capacitySection.htmlContent).toContain('&lt;script&gt;');
    }
  });
  ...
  it('should handle missing optional variables', async () => {
    ...
    // Ensure affirmant_capacity is provided (mapped from affiant.capacity usually)
    context.variables.affirmant_capacity = 'Sui Juris';
